"""
================================================================================
RELABEL + EXTRACT (LITE) train_router_v2.py CORE5 
================================================================================

 tools/relabel_and_extract.py , forward
 router ** generate (max_new_tokens=1)
 relabel , ~50% GPU 

 router ( train_router_v2.py DEFAULT_FEATS):
    metadata:
        id, image_path, label            # label = 'ori_correct' / 'ori_wrong'
        question, labels, category # / 
        ori_pred, ori_raw, ori_correct # relabel 
        model_type
    CORE5 features:
        answer_topp ( softmax top1)
        answer_margin ( softmax top1 - top2)
        vocab_full_entropy_norm ( token / log V)
        option_mass ( token )
        logit_gap_opt_nonopt (max logit - max logit)

**** ( relabel_and_extract.py):
    - p_options / option_probs / answer_entropy / answer_entropy_norm
    - vocab_full_entropy / vocab_top10_entropy
    - vatt_* ( output_attentions forward)
    - has_spatial_keyword / has_fine_detail_keyword / question_length_tokens
    - parsed_options / num_options / num_visual_tokens

:
  <out_dir>/
    ori_preds_rank{r}.jsonl # ori + CORE5 (per-rank shard)
    ori_preds_all.jsonl # jsonl
    features_train_v2.jsonl # train_router_v2.py 
    ori_correct.json / ori_wrong.json # ori_pred==labels 
...
_INTERNVL = os.path.join(_HIDE_ROOT, "Internvl") # OpenGVLab 
...
class InternVL3Adapter(BaseVLMAdapter):
    """ Internvl/cycle_inference_internvl.py OpenGVLab :
      - AutoModel + AutoTokenizer (trust_remote_code=True)
      - AutoProcessor / apply_chat_template
      - Internvl/utiles_internvl.py load_image_from_path / get_input 
        pixel_values input_ids ( patch + thumbnail + IMG_CONTEXT )
      - model.generate(pixel_values=, input_ids=, attention_mask=, ...)
         token logits

     ckpt: OpenGVLab/InternVL3-8B-Instruct * -hf * 
    """
    def load(self, model_path, device, max_pixels, **kw):
        import torch
    ...
    def build_inputs(self, messages):
        """messages :
         ...
         image_path question, OpenGVLab load_image_from_path
        + get_input (pixel_values, input_ids, attention_mask)
        """
        import torch
        image_path = None
        text = ""
...
def build_features_train_jsonl(preds, out_path):
    """ train_router_v2.py jsonl: label + CORE5 + id/image_path"""
