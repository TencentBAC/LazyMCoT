...
# ═══════════════════════════════════════════════════════════════════════════
# 颜色调色盘 + LPD 外侧图注工具（与 Qwen2.5/Get_box.py 同步）
# ═══════════════════════════════════════════════════════════════════════════
...
def batch_messages2out(model, processor, batched_inputs):
    """
    ...
    Returns:
        output_texts: list of str，每条数据的生成文本
    """
    batched_inputs = batched_inputs.to(model.device)
...
def messages2att(model,processor,inputs):
    ...
    with torch.no_grad():
        # 本工程自定义 modeling 里用 chunked_attention 实现 output_attentions，
        # 但 transformers 4.57 会对 flash_attention_2 + output_attentions=True 发 warning，
        # 此处局部 suppress，仅抑制这一次 forward 的告警，不影响其他模块。
...
def compact_and_center_with_relative_pos(imgidx, img_nums, image, normalized_bboxes, n=1,
    ...
    if x_coords:
        x_pix_map[x_coords[-1]] = x_map[x_coords[-1]]
    for i in range(len(y_coords) - 1):
    ...
    if y_coords:
        y_pix_map[y_coords[-1]] = y_map[y_coords[-1]]
