...
def get_sam3_supplement_bboxes(sam3_results, max_per_entity=3, nms_iou_threshold=0.7):
    """
    从 SAM3 文本提示检测结果中提取所有边界框及对应实体标签。

    - 每个实体按面积从大到小排序，最多取 max_per_entity 个。
    - 跨实体 NMS 去重：不同实体检测到同一目标（IoU > nms_iou_threshold）时，
      只保留面积更大的那个，避免高度重叠的重复框。

    Args:
        sam3_results: dict, {entity_name: [[x0, y0, x1, y1], ...]}，坐标归一化到 [0,1]
        max_per_entity: int，每个实体最多保留的 bbox 数量
        nms_iou_threshold: float，跨实体去重的 IoU 阈值
    ...
    for entity, boxes in sam3_results.items():
        ...
        for b in sorted_boxes[:max_per_entity]:
            ...
            candidates.append((b, entity, area))

    # 第二步：按面积降序排列后做跨实体 NMS
    candidates.sort(key=lambda x: x[2], reverse=True)
    supplement_bboxes = []
    entity_labels = []
    for bbox, label, area in candidates:
        # 检查与已保留框的 IoU
        is_dup = False
        for kept_bbox in supplement_bboxes:
            if _bbox_iou(bbox, kept_bbox) > nms_iou_threshold:
                is_dup = True
                break
        if not is_dup:
            supplement_bboxes.append(bbox)
