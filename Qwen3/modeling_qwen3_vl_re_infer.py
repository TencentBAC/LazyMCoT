        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if output_attentions:
            def chunked_attention(module, query_states, key_states, head_dim, target_indices=None, chunk_size=512, attention_mask=None):
                ...
                # print("key_states shape: ", new_key_states.shape)
                B, H, Q_LEN, D = query_states.shape
                _, _, K_LEN, _ = new_key_states.shape
            ...
            # print("target_indices",target_indices)
            attn_weights = chunked_attention(self ,query_states, key_states, self.head_dim, target_indices, chunk_size=128)
        else:
            attn_weights = None
        return attn_output, attn_weights
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        layernums = 0
        ori_output_attentions = output_attentions
        output_attentions = False
        attentions_list = []
        # decoder layers
        for layer_idx, decoder_layer in enumerate(self.layers):
            layernums+=1
            if USE_ALLATT and ori_output_attentions:
                output_attentions = True
            else:
                if layernums in SELECT_LAYER and ori_output_attentions:
                    output_attentions = True
                else:
                    output_attentions = False
            if output_attentions:
