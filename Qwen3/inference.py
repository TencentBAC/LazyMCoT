...
# ──────────────────────────────────────────────────────────────────────────────
# GRACE 双套图工具：兼容旧 5 元组 / 新 6 元组的 att_results 解包，
# 以及最终推理前按"原图 → HiDe LPD → GRACE LPD"顺序追加视觉输入
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# Router v2 辅助：从完整词表 first_logits 抽取 v2 特征
# ──────────────────────────────────────────────────────────────────────────────
...
def compute_router_v2_features(first_logits, question, option_token_ids_all):
    ...
    计算 RouterV2 可能用到的数值特征（7 维完备集合，router 按自身需要 pick）。
...
def cycle_epoch_infer(gpu_id, rank, dataset_part, savedir, max_pixels, sig, thre, batch_size=4,
    ...
    参数说明:
        ...
        enable_router: 是否启用难度感知路由（RouterV2 / Conformal / 4 类）
        router_report_path: router 配置文件路径（如 router_v2_gbdt.json）
        router_alpha: 覆盖 router 训练时的 α，按新 α 重算 s_floor
        heatmap_save_dir: str | None，注意力热力图 / LPD 输出图保存目录。
            None（默认）表示不保存；非 None 时每条样本每张图都会保存。
    """
    ...
    print(f"  创新点开关: EGAF={enable_saaa}(mode={egaf_fusion_mode},w={egaf_expert_weight}), ACR={enable_acr}, PMGVV={enable_pmgvv}, GRACE={enable_grace}, ROUTER={enable_router}")
    ...
    for batch_start in tqdm(range(0, num_samples, batch_size), desc=f"GPU{gpu_id} batches"):
        ...
        # 同一 prompt。若 router 未启用或是 v1 conformal/binary，则保持原 tag prompt。
        ...
        if skip_ori:
            ...
            try:
                ...
                # ⚠️ 为保持与原 GRACE 流程**GPU 状态/数值一致**，
                # direct-answer 的 ori 生成与 router 特征采集 **必须分开做两次 forward**：
                #   (1) 先用 **不带 output_scores** 的 generate 拿 ori 答案
                #   (2) 再用单独的 1-token forward 拿 first_logits 给 router

                # ── Step 1: 拿 ori 答案（不带 output_scores） ──
            ...
            if answer_out:
                ...
                for s in sig:
                    for t in thre:
                        ...
                        # 最终推理：按"原图 → HiDe LPD → GRACE LPD"顺序追加图像
